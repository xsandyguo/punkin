...
char* UTF8::NextCharacter(const char* str, jint* value) {
    unsigned const char *ptr = (const unsigned char *)str;
    /* See if it's legal supplementary character:
       11101101 1010xxxx 10xxxxxx 11101101 1011xxxx 10xxxxxx */
    if (IsSupplementaryCharacter(ptr)) {
        *value = GetSupplementaryCharacter(ptr);
        return (char *)(ptr + 6);
    }
    jchar result;
...
int UTF8::GetUnicodeLength(const char* str, int len) {
    int num_chars = len;
    for (int i = 0; i < len; i++) {
        if ((str[i] & 0xC0) == 0x80) {
            --num_chars;
...
int UTF8::GetUnicodeLength(const char* str) {
    int num_chars = 0;
...
static u1* utf8_write(u1* base, jchar ch) {
    if ((ch != 0) && (ch <= 0x7f)) {
        base[0] = (u1)ch;
        return base + 1;
    }
...
void UTF8::ConvertToUnicode(const char* utf8_str, jchar* unicode_str, int unicode_length) {
    unsigned char ch;
...
int UTF8::GetQuotedAsciiLength(const char* utf8_str, int utf8_length) {
    const char *ptr = utf8_str;
...
void UTF8::AsQuotedAscii(const char* utf8_str, int utf8_length, char* buf, int buflen) {
    const char *ptr = utf8_str;
...
const char* UTF8::FromQuotedAscii(const char* quoted_ascii_str) {
        ...
        while (*ptr != '\0') {
            if (*ptr != '\\') {
                ...
                case 'u': {
                    ...
                    if (buffer == NULL) {
                        char utf8_buffer[4];
                        char* next = (char*)utf8_write((u1*)utf8_buffer, value);
                        length += next - utf8_buffer;
                    } else {
                        char* next = (char*)utf8_write((u1*)&buffer[length], value);
                ...
                case 'n':
                    ...
                    break;
                case 'r':
                    if (buffer != NULL) buffer[length] = '\r';
                    ptr += 2;
                    length++;
                    break;
                case 'f':
                    if (buffer != NULL) buffer[length] = '\f';
                    ptr += 2;
                    length++;
                    break;
                default:
...
bool UTF8::IsSupplementaryCharacter(const unsigned char* str) {
    return ((str[0] & 0xFF) == 0xED) && ((str[1] & 0xF0) == 0xA0) && ((str[2] & 0xC0) == 0x80)
           && ((str[3] & 0xFF) == 0xED) && ((str[4] & 0xF0) == 0xB0) && ((str[5] & 0xC0) == 0x80);
}

jint UTF8::GetSupplementaryCharacter(const unsigned char* str) {
...
int Unicode::GetUtf8Size(jchar c) {
    if ((0x0001 <= c) && (c <= 0x007F)) return 1;
    if (c <= 0x07FF) return 2;
    return 3;
}

int Unicode::GetUtf8Length(jchar* base, int length) {
    int result = 0;
...
char* Unicode::AsUtf8(jchar* base, int length) {
    int utf8_len = GetUtf8Length(base, length);
    u1* result = new u1[utf8_len + 1];
    u1* p = result;
    for (int index = 0; index < length; index++) {
...
char* Unicode::AsUtf8(jchar* base, int length, char* buf, int buflen) {
    u1* p = (u1*)buf;
    u1* end = (u1*)buf + buflen;
    for (int index = 0; index < length; index++) {
        jchar c = base[index];
        if (p + GetUtf8Size(c) >= end) break;      // string is truncated
        p = utf8_write(p, base[index]);
    }
    *p = '\0';
    return buf;
...
void Unicode::ConvertToUtf8(const jchar* base, int length, char* utf8_buffer) {
    for (int index = 0; index < length; index++) {
        utf8_buffer = (char*)utf8_write((u1*)utf8_buffer, base[index]);
    }
    *utf8_buffer = '\0';
}

// returns the quoted ascii length of a unicode string
int Unicode::GetQuotedAsciiLength(jchar* base, int length) {
    int result = 0;
...
// converts a utf8 string to quoted ascii
void Unicode::AsQuotedAscii(const jchar* base, int length, char* buf, int buflen) {
    char* p = buf;
    char* end = buf + buflen;
    for (int index = 0; index < length; index++) {
        jchar c = base[index];
